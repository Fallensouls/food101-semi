...
#     if args.dataset == 'food101unlabel':
#         unlabeled_dataset, test_dataset = DATASET_GETTERS[args.dataset](
#             './food-101')
...
#     predictor = MLPHead(128, 512, 128).to(args.device)
...
#           model, optimizer, ema_model, scheduler, writer, predictor)

#     train_linear(args, labeled_trainloader, unlabeled_trainloader, test_loader,
...
#             try:
#                 (inputs_u_w, inputs_u_s, y1, y2), _ = unlabeled_iter.next()
#             except:
#                 unlabeled_iter = iter(unlabeled_trainloader)
#                 (inputs_u_w, inputs_u_s, y1, y2), _ = unlabeled_iter.next()
...
#             Lbyol = cal_byol_loss(model, ema_model.ema, predictor, y1.to(args.device, non_blocking=True), y2.to(args.device, non_blocking=True))
...
#             if not args.no_progress:
#                 p_bar.set_description("Train Epoch: {epoch}/{epochs:4}. Iter: {batch:4}/{iter:4}. LR: {lr:.4f}. Loss: {loss:.4f}. Loss_b: {loss_b:.4f}. ".format(
#                     epoch=epoch + 1,
#                     epochs=args.epochs,
#                     batch=batch_idx + 1,
...
# def train_linear(args, labeled_trainloader, unlabeled_trainloader, test_loader,
#           model, ema_model, scheduler, writer):
#     if args.amp:
#         from apex import amp
#     global best_acc
...
#             # writer.add_scalar('train/4.train_loss_b', losses_b.avg, epoch)
#             writer.add_scalar('train/5.mask', mask_prob, epoch)
...
# def cal_byol_loss(online_network, target_network, predictor, batch_view_1, batch_view_2):
#     # compute query feature
#     _, output1 = online_network(batch_view_1)
#     _, output2 = online_network(batch_view_2)
#     predictions_from_view_1 = predictor(output1)
#     predictions_from_view_2 = predictor(output2)

#     # compute key features
#     with torch.no_grad():
#         _, targets_to_view_2 = target_network(batch_view_1)
#         _, targets_to_view_1 = target_network(batch_view_2)

#     loss = regression_loss(predictions_from_view_1, targets_to_view_1)
#     loss += regression_loss(predictions_from_view_2, targets_to_view_2)
#     return loss.mean()
